...
function collisions()
{
    ...
    {
        let [collision, force] =  CollisionManager.SATCollision(bombers[b].getRect.getPoints(),player.getShape.getPoints())
        if(collision)
        {
            //player.getAcceleration.addVec = new Vec2(-player.getAcceleration.x *2,player.getAcceleration.y *2);
            //impulse
