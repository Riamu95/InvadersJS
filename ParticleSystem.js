...
class ParticleSystem
{
    ...
    render(ctx)
    {
       for(let p =  this._particles.length - 1; p >= 0; p--)
       {
            ...
            ctx.moveTo(this._particles[p].getPoints()[0].x , this._particles[p].getPoints()[0].y);
        
            for( let i = 1; i < this._particles[p].getPoints().length; i++)
            {
                ctx.lineTo(this._particles[p].getPoints()[i].x, this._particles[p].getPoints()[i].y);
            }

            ctx.lineTo(this._particles[p].getPoints()[0].x , this._particles[p].getPoints()[0].y);
            ctx.fill();
            ctx.closePath();
            ctx.restore();
       }   
